...
const Swatch = ({ color, scale }: { color: string; scale?: string }) => {
  ...
  return (
    <Tooltip content="Copied" isOpen={copied}>
      <Button
        className="m-2 flex h-24 w-24 flex-col items-center justify-center gap-1 rounded-medium shadow-small"
        style={{
...
const SemanticSwatch = ({
  ...
  return (
    <Tooltip content="Copied" isOpen={copied}>
      <Button
        className={`${className} m-2 flex h-24 w-24 flex-col items-center justify-center gap-1 rounded-medium shadow-small`}
        onPress={handleCopy}
      >
        <span className={`${textClassName} font-semibold text-tiny`}>
          {color}
        </span>
        {!isEmpty(value) && (
          <span className={`${textClassName} font-semibold text-tiny`}>
  ...
  <div className="flex flex-row flex-wrap items-center justify-center">
    {colors.map(({ title, items }) => (
      <div key={title} className="flex h-full w-full flex-col items-start">
        ...
        <div className="flex h-full w-full flex-row flex-wrap items-center justify-start px-4 py-1">
          {items.map((c, index) =>
            isSemantic ? (
              <SemanticSwatch
                key={`${c.color}-${index}`}
