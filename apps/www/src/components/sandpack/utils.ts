...
import type { HighlightedLine, HighlightedLines } from './types';
...
const getLines = (lines?: string): HighlightedLine => {
  ...
  const count = end
    ? Number.parseInt(`${end}`, 10) - Number.parseInt(`${start}`, 10) + 1
    : 0;
...
export const getHighlightedLines = (
  ...
  return Array.from({ length: lines.count }, (_, i) => ({
    className: 'sp-highlight',
    line: Number.parseInt(`${lines.start}`, 10) + i,
  }));
};
...
export function getNextUIComponents(files: SandpackFiles): {
  ...
  if (lineIndex !== -1) {
    ...
    componentNames.forEach((componentName) => {
      ...
      lines.splice(
        lineIndex,
        0,
        `  "./node_modules/@heroui/theme/dist/components/${name}.js",`
      );
    });
  }
